...
from .pgcan_utils import Model_PGCAN, Model_PGCAN3D, Encoder, Encoder_conv_3d, NetworkM4_fused
...
class LMGP(GPR):
    ...
    def __init__(
        ...
        self.dz = Training['dz'] ####### ADDED
        self.Nx = Training['Nx'] ####### ADDED
        self.Ny = Training['Ny'] ####### ADDED
        self.Nz = Training['Nz'] ####### ADDED
        ...
        self.elem_x = Training['X_col_elem'] ####### ADDED
        self.mask_col_elem = Training['mask_col_elem'] ####### ADDED
        self.detJ = Training['detJ'] ####### ADDED
        self.dN = Training['dN'] ####### ADDED
        self.mask_nan = Training['mask_nan'] ####### ADDED
        self.xi = Training['xi'] ####### ADDED
        self.yi = Training['yi'] ####### ADDED
        self.zi = Training['zi'] ####### ADDED
        ...
        self.groups = NN_config['groups']
        self.domain_size = [MP['domain']['x'][0],MP['domain']['x'][1],MP['domain']['y'][0],MP['domain']['y'][1],MP['domain']['z'][0],MP['domain']['z'][1]]
        ...
        if self.basis=='single':
            ...
            setattr(self,'mean_module_NN_All', NetworkM4(input_dim = train_x.shape[1], output_dim=num_output, layers = self.NN_layers_base))     
        ...
        elif self.basis == 'PGCAN3D':
             network =  NetworkM4_fused(input_dim = train_x.shape[1], output_dim=num_output, layers = self.NN_layers_base)
             encoder = Encoder_conv_3d(
                                n_features=self.n_features,  
                                res=self.res,                      
                                n_cells=self.n_cells,         
                                domain_size=self.domain_size,
             ...
             setattr(self,'mean_module_NN_All', Model_PGCAN3D(network = network , encoder = encoder))

        # Fix the hyperparameter value
        self.covar_module.base_kernel.raw_lengthscale.data = torch.tensor([self.omega, self.omega, self.omega])  # Set the desired value
