...
class StructuralFE:
    ...
    def solve88(self, density):
        ...
        EnetOfElem = np.array([np.dot(self.EMaterials,(density[i,:])**self.penal) for i in range(density.shape[0])]);
    ...
    def computeElementStrains(self):
        ...
        strainUV = 0.5*(0.5*(delta[:,6] - delta[:,0] + delta[:,4]-delta[:,2])/s \
    ...
    def plotFE(self):
         ...
         rhoElem = np.array([np.max(self.densityField[i,:]) for i in range(self.densityField.shape[0])]);
         ...
         # plt.subplot(2,2,2);
         # im = plt.imshow(np.fliplr(np.flipud(rhoElem*elemV).reshape((self.nelx,self.nely)).T), cmap=cm.jet,interpolation='none')
         # fig.colorbar(im)
         # plt.title('|U_y|')

         # displacement = np.sqrt(elemU**2 + elemV**2);
         # plt.subplot(2,2,3);
         # im = plt.imshow(np.fliplr(np.flipud(rhoElem*displacement).reshape((self.nelx,self.nely)).T), cmap=cm.jet,interpolation='none')
         # plt.title('||U||')
         # fig.colorbar(im)

         # plt.subplot(2,2,4);
         # im = plt.imshow(np.fliplr(np.flipud(rhoElem*self.Jelem).reshape((self.nelx,self.nely)).T), cmap=cm.jet,interpolation='none')
         # fig.colorbar(im)
         # plt.title('Jelem')
         # fig.show()
...
def runFEA():
    ...
    if(problem == 1): # half MBB
        ...
        fixed = nely + np.array([(nelx+1)*(nely+1) , (nelx+1)*(nely+1)-1, (nelx+1)*(nely+1) - (nely) , (nelx+1)*(nely+1)-1 - (nely)]);
