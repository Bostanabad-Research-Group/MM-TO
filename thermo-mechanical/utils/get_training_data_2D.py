...
def get_data_EX2D1_thermomech_gripper_four_phase_source(MP):
    ...
    file_loc = base_folder + '/Data/' + f'{Example}_GPU{N_worker}_{Diff_type}_thermomechanical.pt'
    ...
    for i, device in enumerate(tkwargs["devices"]):
        ...
        Training_multi[key] = {
            ...
            "vd1_X_train": Training["vd1_X_train"].type(tkwargs["dtype"]).to(device).requires_grad_(False),
            "vd2_X_train": Training["vd2_X_train"].type(tkwargs["dtype"]).to(device).requires_grad_(False),
            "vt_X_train": Training["vt_X_train"].type(tkwargs["dtype"]).to(device).requires_grad_(False),
            ...
            "vd1_train": Training["vd1_train"].type(tkwargs["dtype"]).to(device).requires_grad_(False),
            "vd2_train": Training["vd2_train"].type(tkwargs["dtype"]).to(device).requires_grad_(False),
            "vt_train": Training["vt_train"].type(tkwargs["dtype"]).to(device).requires_grad_(False),
...
def get_data_EX2D1_thermomech_gripper_two_phase_source(MP):
    Example = MP['Example']
    Diff_type = MP['Diff_type']
    N_worker = MP['N_worker']
    base_folder = MP['base_folder']
    ...
    Training = {'u_X_train':u_X_train,'u_train':u_train,'v_X_train':v_X_train,'v_train':v_train,
                ...
                'vd1_X_train':vd1_X_train,'vd1_train':vd1_train,'vd2_X_train':vd2_X_train,'vd2_train':vd2_train,
                'vt_X_train':vt_X_train,'vt_train':vt_train,
                'phase0_X_train':phase0_X_train,'phase0_train':phase0_train,'phase1_X_train':phase1_X_train,'phase1_train':phase1_train,}
...
def get_data_EX2D2_thermomech_actuator_two_phase_source(MP):
    Example = MP['Example']
    Diff_type = MP['Diff_type']
    N_worker = MP['N_worker']
    base_folder = MP['base_folder']
    ...
    phase1_X_train = torch.tensor(tp[0:3]).type(tkwargs["dtype"]).requires_grad_(False)
    phase0_train = (torch.zeros_like(phase0_X_train)[:,0]).type(tkwargs["dtype"])
    phase1_train = (torch.ones_like(phase1_X_train)[:,0]).type(tkwargs["dtype"])
