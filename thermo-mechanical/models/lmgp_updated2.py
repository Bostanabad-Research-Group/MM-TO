        ...
        if self.basis=='neural_network':
            ############################################### One NN for ALL
             ...
             if self.name_output == 'rho': # density network
                network =  NetworkM4_fused_rho(input_dim = base_train_x.shape[1], output_dim=num_output, layers = self.NN_layers_base, activation = self.activation, init_method = self.init_method)
             else: # displacement or temperature network
