...
from .pgcan_utils import Model_PGCAN , Encoder , NetworkM4_fused_disp, NetworkM4_fused_rho
...
class LMGP(GPR):
    ...
    def __init__(
        ...
        self.elem_x = Training['X_col_elem']
        ...
        self.xi_elem = Training['xi_elem'] ####### ADDED
        self.yi_elem = Training['yi_elem'] ####### ADDED
        ...
        if self.basis=='neural_network':
            ...
            setattr(self,'mean_module_NN_All', NetworkM4(input_dim = train_x.shape[1], output_dim=num_output, layers = self.NN_layers_base))     
             ...
             if self.name_output == 'rho': # density network
                network =  NetworkM4_fused_rho(input_dim = train_x.shape[1], output_dim=num_output, layers = self.NN_layers_base, activation = self.activation, init_method = self.init_method)
             else: # displacement network
                network =  NetworkM4_fused_disp(input_dim = train_x.shape[1], output_dim=num_output, layers = self.NN_layers_base, activation = self.activation, init_method = self.init_method)
             encoder = Encoder(
