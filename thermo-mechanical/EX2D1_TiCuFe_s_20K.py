...
from utils.get_training_data_2D import get_data_EX2D1_thermomech_gripper_four_phase_source as get_data
...
if __name__ == "__main__":
    ...
    design_flag = True # wether we have design constraint or not
    ...
    kappa = [1.0e-8, 2.59e-5, 40e-5, 6e-5] # thermal conductivity: W/(K um)   Ni, steel, Al, Copper Ti: 9.07e-5, 6e-5, 23.7e-5, 40e-5, 2.59e-5
    alpha = [1.2e-5, 0.86e-5, 1.7e-5, 1.2e-5,] # thermal expansion coefficient: Ni, steel, Al, Copper, Ti: 1.5e-5, 1.2e-5, 2.3e-5, 1.7e-5, 0.86e-5
    deltaT = 100 # isothermal temperature increment 
    D = np.array([0, 4.5, 8.96, 7.8])  # density g/cm^3: Ni, steel, Al, ti, Copper: 8.9, 7.8, 2.7, 8.96, 4.5
    D = D/D.max() # normalize D with the maximum value
    E = [1e-5,0.12,0.128,0.2]  # modulus GP/1000: Ni, steel, Al, Copper, ti: 0.2, 0.2, 0.07, 0.128, 0.12
