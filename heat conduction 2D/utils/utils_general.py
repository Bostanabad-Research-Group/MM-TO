...
def get_multiGPU(N_worker):
    """
    Returns a dictionary with CUDA multi-GPU configuration.
    N_worker is the requested number of GPUs to use.
    Raises an error if the available GPU count does not match N_worker.
    """
    ...
    if available_gpus != N_worker:
        raise ValueError(
            f"Requested {N_worker} GPUs, but {available_gpus} available."
        )

    devices = [torch.device(f"cuda:{i}") for i in range(N_worker)]
